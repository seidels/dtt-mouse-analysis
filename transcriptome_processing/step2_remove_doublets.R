...
fd = fd[(fd$gene_type %in% c('protein_coding', 'pseudogene', 'lncRNA')) & (!fd$chr %in% c('chrX', 'chrY')),]
...
for(i in 1:batch_num){
    ...
    count = count[fd$gene_ID, rownames(pd)]
...
#####################################################################
### calculating top10 differential expressed genes for each partition
...
########################################################################################
### we take the top10 genes of each partition to perform subclustering on each partition
...
#for(cluster_i in cluster_list){
    ...
    medians = pd %>% group_by(seurat_clusters) %>% summarize(med = median(doublet_score)) %>%
        mutate(fill_color = ifelse(med > 0.025, "red", "gray"))
...
for(i in 1:max(obj$my_partition)){
    print(i)
    exclude_cluster_list = c(-1)
    if(i == 1){exclude_cluster_list = c(49)}
    if(i == 3){exclude_cluster_list = c(14,18,19)}
    if(i == 4){exclude_cluster_list = c(7)}
    if(i == 5){exclude_cluster_list = c(11)}
    if(i == 6){exclude_cluster_list = c(6,10)}
    if(i == 8){exclude_cluster_list = c(9,10)}
